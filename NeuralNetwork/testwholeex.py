import json 
import numpy as np 
import pandas as pd  
from tqdm import tqdm

class Neuron:     
    def __init__(self, weights, bias):         
        self.weights = np.array(weights)         
        self.bias = bias         
        self.alpha = 0.01      
        
    def activation(self, x):         
        return np.maximum(self.alpha * x, x)      
        
    def calculate(self, input):         
        input = np.array(input)         
        return self.activation(np.dot(self.weights, input) + self.bias)  

class Outputneuron:     
    def __init__(self, weights, bias):         
        self.weights = np.array(weights)         
        self.bias = bias      
        
    def calculate(self, inputs):         
        inputs = np.array(inputs)         
        self.unactivated_sum = np.dot(self.weights, inputs) + self.bias         
        return self.unactivated_sum  
    
    def softmax(self, x):         
        exp_x = np.exp(x - np.max(x))         
        return exp_x / np.sum(exp_x)  

class Hiddenlayers:     
    def __init__(self, hiddenLayers):         
        self.hiddenLayers = hiddenLayers      
        
    def forwordProp(self, input):         
        for layer in self.hiddenLayers:             
            input = np.array([neuron.calculate(input) for neuron in layer])         
        return input  

class Outputlayer:     
    def __init__(self, layer):         
        self.layer = layer      
        
    def calculate(self, input):
        logits = np.array([neuron.calculate(input) for neuron in self.layer])
        exp_x = np.exp(logits - np.max(logits))
        return exp_x / np.sum(exp_x)

class Network:     
    def __init__(self):         
        self.hiddenLayers, self.outputLayer = self.initialize()      
        
    def initialize(self):         
        with open("NeuralNetwork/best_model.json", 'r') as file:             
            network = json.load(file)          
            
        hiddenLayersWrights = network["hidden_layer_weights"]         
        hiddenLayersBiases = network["hidden_layer_biases"]         
        hiddenLayers = []          
        
        for i in range(len(hiddenLayersWrights)):             
            hiddenLayerWeights = hiddenLayersWrights[i]             
            hiddenLayerBiases = hiddenLayersBiases[i]             
            layer = [Neuron(hiddenLayerWeights[j], hiddenLayerBiases[j]) for j in range(len(hiddenLayerWeights))]             
            hiddenLayers.append(layer)          
            
        outputLayerWeights = network["output_layer_weights"]         
        outputLayerBiases = network["output_layer_biases"]         
        outputLayer = [Outputneuron(outputLayerWeights[i], outputLayerBiases[i]) for i in range(len(outputLayerWeights))]          
        
        return Hiddenlayers(hiddenLayers), Outputlayer(outputLayer)      
        
    def predict(self, input):         
        hiddenOutput = self.hiddenLayers.forwordProp(input)         
        prediction = self.outputLayer.calculate(hiddenOutput)         
        return prediction      
        
    def test(self, testexamples):         
        correct = 0         
        total = 0
        example_bar=tqdm(testexamples,unit="example")
        for testexample in example_bar:             
            label = testexample[0]             
            pixels = testexample[1:] / 255.0             
            prediction = self.predict(pixels)             
            if np.argmax(prediction) == label:                 
                correct += 1             
            total += 1         
        accuracy = correct / total         
        print(f"Iteration {total} |correct {correct}| Accuracy: {accuracy:.4f}")  
print("Loading model and initalizing the network")
network = Network() 
print("Network initialized")
data = pd.read_csv('NeuralNetwork/mnist_train.csv').to_numpy() 
network.test(data)